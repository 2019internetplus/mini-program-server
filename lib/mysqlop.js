...
function getAllEmotionValue(openid){
      return new Promise((resolve, reject) => {
            var handler = conn.conn;
            var sql = "SELECT total_value, input_value, test_value, self_affirm, anti_anxiety, anti_melancholy, message, time FROM em WHERE open_id = ?"
            var params = [openid];
            handler.query(sql, params, function(err, result, fields){
                  if(err) reject(err);
                  resolve({
...
exports.getAllEmotionValue = getAllEmotionValue;

function getTodayEmotionValue(openid){
      return new Promise((resolve, reject) => {
            user.getCommitSession(openid).then((value) => {
                  if(!value) reject("value is null");
                  ...
                  var handler = conn.conn;
                  var sql = "SELECT * FROM em WHERE open_id = ? AND time = ?"; //TODO
           
                  var params = [openid, parseInt(value)];
...
exports.getTodayEmotionValue = getTodayEmotionValue;
...
function valueCompute(openid, time){
      ...
      select.then((value) => {
            ...
            if(value !== null){
                  ...
                  var handler = conn.conn;
                  let sql = "UPDATE em SET total_value = ? WHERE open_id = ? AND time = ?";
...
// +-----------+------------+---------+-----------+---------+---------------+---------+-----------+
// |   u_id    |  max_value | max_day | min_value | min_day | average_value |  is_pu  | variance  |
// +-----------+------------+---------+-----------+---------+---------------+---------------------+

// Query both em table and weekly_report table 
function getWeeklyData(openid, from, to){
      return new Promise((resolve, reject) => {
      var handler = conn.conn;
      var sql = "SELECT total_value FROM em WHERE open_id = ? AND time <= to AND time >= from"; 
      var params = [openid, parseInt(from), parseInt(to)];
      return new Promise((resolve, reject) =>{
            handler.query(sql, params, function(err, result, fields){
                  if(err) reject(err);
                  resolve({
...
function getWeeklyReport(openid, from, to){
      
            var handler = conn.conn;
            var sql = "SELECT max_value, max_day, min_value, min_day, average, variance, from_value, to_value FROM weekly_reports WHERE open_id = ? AND time = to AND time = from"; 
            var params = [openid, parseInt(from), parseInt(to)];
            return new Promise((resolve, reject) =>{
                  handler.query(sql, params, function(err, result, fields){
                        if(err) reject(err);
...
function getWeeklyReports(openid){
      var handler = conn.conn;
      var sql = "SELECT max_value, max_day, min_value, min_day, average, variance, from_value, to_value, count FROM weekly_reports WHERE open_id = ?"; 
      var params = [openid];
      return new Promise((resolve, reject) =>{
            handler.query(sql, params, function(err, result, fields){
                  if(err) reject(err);
...
// |  openid | message | time |
// +---------+---------+------+
function addFeedback(openid, message){
      return new Promise((resolve, reject) => {
            var handler = conn.conn;
            const time = new Date().valueOf();
            var sql = "INSERT INTO feedback_info(openid, message, time) VALUES(?, ?, ?)"; 
