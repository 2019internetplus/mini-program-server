 ...
 */
 ...
 // flist
 // +---------+--------+---------+-------------+
 // | flistid |  title | content | flist_index |
 // +---------+--------+---------+-------------+
...
function getFictionsList(start, count){
      return new Promise((resolve, reject) => {
            var handler = conn.conn;
            var sql = "SELECT content, title FROM flist limit ? offset ? ";
...
exports.getFictionsList = getFictionsList;
...
function addFiction(title, content){
      return new Promise((resolve, reject) => {
            var handler = conn.conn;
            var sql = "INSERT INTO flist (title, content) VALUES(?, ?) ";
 ...
 * @param {Int} nickId 
 */
function cancelLikeKua(kuaId, nickId){
      return new Promise((resolve, reject) => {  
            var handler = conn.conn;
            var sql = "DELETE FROM kua_like_info WHERE kua_id = ? and nick_id = ?";
            
            var params = [parseInt(kuaId), parseInt(nickId)];
...
exports.cancelLikeKua = cancelLikeKua;
/**
 * Add comment
 * @param {Int} kuaId 
 ...
 */
 ...
 //test
 // +---------+---------+------+-----+-----+
 // | test_id | p_title | s_1  | s_2 | s_3 |
 // +---------+---------+------+-----+-----+
 //user_test_info
 // +---------+---------+-----------+-------+
 // | nick_id | test_id | test_time | score |
 // +---------+---------+-----------+-------+
